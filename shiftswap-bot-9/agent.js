...
async function openDM(client, userId) {
  try {
    ...
    console.error("❌ conversations.open failed for " + userId + ":", err.message);
    throw err;
  }
}
...
async function send(client, userId, text) {
  try {
    ...
    // Fallback: post directly to user ID as channel
    try {
      await client.chat.postMessage({ channel: userId, text });
      console.log("✅ Direct post succeeded for " + userId);
    } catch (err2) {
      console.error("❌ Direct post also failed:", err2.message);
    }
  }
}

// Post in a shared channel and mention the user
// This works even if the user has never messaged the bot
async function sendViaChannel(client, teamId, userId, userName, text) {
  try {
    // First try DM
    const ch = await openDM(client, userId);
    await client.chat.postMessage({ channel: ch, text });
    ...
    console.log("⚠ DM failed for " + userName + ", trying shared channel");
  }

  // Fallback: find a channel the bot and user are both in
  try {
    ...
    const botChannels = await client.conversations.list({ types: "public_channel,private_channel", exclude_archived: true });
    const botChannelIds = new Set((botChannels.channels || []).map(c => c.id));

    const shared = channels.find(c => botChannelIds.has(c.id));
    if (shared) {
      await client.chat.postMessage({
        channel: shared.id,
        text: "<@" + userId + "> " + text,
      });
      console.log("✅ Sent via shared channel #" + shared.name + " to " + userName);
      return;
    }
  } catch (err) {
    console.error("❌ Shared channel fallback failed:", err.message);
  }

  console.error("❌ Could not reach " + userName + " by any method");
}
