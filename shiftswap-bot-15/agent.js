...
async function sendToCandidate(client, candidateId, candidateName, text) {
  // Always use the BOT token so candidate replies come back to the bot.
  // User token sends would route replies to a personal DM the bot never sees.
  ...
  // Fallback: shared channel @mention — bot is in the channel so it sees replies
  try {
...
async function handleMessage({ teamId, message, client }) {
  ...
  console.log("MSG [" + teamId + "] " + userId + " (" + channelType + "): " + text.slice(0, 80));
  ...
  // In a channel — only process if this person is an active candidate
  // and the message is in the channel the bot reached out in
  if (channelType === "channel") {
    if (!candidateRequest) {
      return; // Not a candidate, ignore channel messages
    }
    // Only respond if they replied in the same channel we messaged them in
    if (candidateRequest.currentAskedChannelId !== message.channel) {
      return;
    }
  }
  ...
  if (candidateRequest) {
    ...
    // Reply in the same channel/DM where the conversation is happening
    if (channelType === "channel") {
      await client.chat.postMessage({
        channel: message.channel,
        text: "<@" + userId + "> " + (result.message || "Got it!"),
      });
    } else {
