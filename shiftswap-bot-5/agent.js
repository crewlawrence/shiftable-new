...
- If outreach is in progress, reassure them you're on it.`,
  ...
  console.log(`📋 Conversation for action decision:\n${fullConversation}`);
  ...
  if (decision.action === "save_shift" && !activeRequest) {
    ...
    const unmatchedNames = nameList.filter(name => {
      const lower = name.toLowerCase().trim();
      return !members.find(m =>
        m.name.toLowerCase() === lower ||
        m.name.toLowerCase().split(" ")[0] === lower
    ...
    if (unmatchedNames.length > 0) {
      await send(client, userId, `Heads up — I couldn't find ${unmatchedNames.join(", ")} in the workspace so I'll skip them.`);
    }
