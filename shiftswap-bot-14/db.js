...
async function updateShiftDetails(teamId, requestId, shiftDetails) {
  await pool.query(
    "UPDATE requests SET shift_details = $1 WHERE team_id = $2 AND id = $3",
    [JSON.stringify(shiftDetails), teamId, requestId]
  );
  return getRequest(teamId, requestId);
}
